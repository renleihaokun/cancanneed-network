...
var SPEED_CITY_ZH = { ...SPEED_CITY_ZH_1, ...SPEED_CITY_ZH_2 };
function localizeSpeedLocations(locations) {
  if (!Array.isArray(locations)) return locations;
  for (const loc of locations) {
    if (loc.region) loc.region = SPEED_REGION_ZH[loc.region] || loc.region;
    if (loc.city) loc.city = SPEED_CITY_ZH[loc.city] || loc.city;
  }
  return locations;
}
...
var src_default = {
  async fetch(request, env) {
    ...
    if (action === "speed_down") {
      return handleSpeedDown(request);
    }
    if (action === "speed_locations") {
      return handleSpeedLocations();
    }
...
async function handleSpeedDown(request) {
  ...
  if (rawBytes != null && rawBytes !== "") {
    ...
    if (!Number.isFinite(bytes) || bytes <= 0) {
      return new Response(JSON.stringify({ error: "bytes \u65E0\u6548" }), {
        status: 400,
    ...
    if (parsed == null) {
      return new Response(
        JSON.stringify({ error: "size \u683C\u5F0F\u4E0D\u6B63\u786E\uFF0C\u4F8B\u5982 10m\u300150mb\u30011g" }),
...
__name(handleSpeedDown, "handleSpeedDown");
async function handleSpeedLocations() {
  const baseHeaders = { "access-control-allow-origin": "*" };
  try {
...
__name(handleSpeedLocations, "handleSpeedLocations");
...
// .wrangler/tmp/bundle-ysMnVy/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ysMnVy/middleware-loader.entry.ts
